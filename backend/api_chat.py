...
async def regenerate():
    """Regenera a resposta para a última pergunta feita"""
    if not chat_chain:
        raise HTTPException(status_code=400, detail="O chatbot ainda não foi inicializado.")
    if not last_question:
        raise HTTPException(status_code=400, detail="Nenhuma pergunta anterior foi feita.")
    
    response = chat_chain.invoke({"question": last_question})
    return {"answer": response["answer"]}
